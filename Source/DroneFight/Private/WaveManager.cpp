...
//현재 웨이브 밸류에 맞게 제일 높은 밸류부터 랜덤 계산 해서 클래스 당 스폰할 총 몬스터 수 계산식, 단 소환 X, 소환 명단 정리만
void AWaveManager::SpawnMonsterValueInWave()
{
	MonsterClassInWave.Empty();
	int WaveValue = StartWaveValue * MultipleWaveValue * (CurrentWave - 1); // 웨이브 밸류 설정
	int CurrentValue = 0; // 현재 밸류
	TArray<TSubclassOf<AActor>> Keys;
	MonsterClassValues.GetKeys(Keys);
	// 내림차순으로 정렬된 MonsterClassValues에서 높은 밸류부터 몬스터 선택
	for (int i = Keys.Num() - 1; i >= 0; i--)
	{
		TSubclassOf<AActor> MonsterClassKey = Keys[i];
		int MonsterValue = MonsterClassValues[MonsterClassKey];
		while (CurrentValue + MonsterValue <= WaveValue)
		{
			MonsterClassInWave.Add(MonsterClassKey);
			CurrentValue += MonsterValue;
		}
	}
...
{
	// 몬스터 클래스 리스트를 무작위로 섞기
	for (int i = 0; i < MonsterClassInWave.Num(); i++)
	{
		int SwapIndex = rand() % MonsterClassInWave.Num();
		if (i != SwapIndex)
		{
