...
void ADroneBase::Tick(float DeltaTime)
{
	...
	else
	{
		...
		if (_toTargetDir.SquaredLength() <= _sqrArriveRange)
		{
			...
			SetActorRotation(_droneContainer->GetActorRotation());
			return;
		}
...
void ADroneBase::Tracing()
{
	...
	// 유니티 원본에는 충돌 감지 및 레이캐스트가 없으므로 제거
	
	// 타겟 위치 (높이 조정)
	FVector _targetPos = _target->GetActorLocation() + FVector(0, 0, _droneHeight);

	// 타겟까지의 방향 및 거리 계산
	FVector _toTarget = (_targetPos - _currentPosition).GetSafeNormal();
	...
	// 위치 업데이트
	SetActorLocation(_currentPosition + GetActorForwardVector() * _moveSpeed * _deltaTime);

	// 회전 업데이트
	FRotator _targetRotation = _toTarget.Rotation();
	
	// 현재 회전값과 목표 회전값 사이를 부드럽게 보간 (회전 속도 적용)
	float _rotationSpeed = _currentRotateSpeed * 0.01f; // 회전 속도 스케일 조정
	FRotator _newRotation = FMath::RInterpTo(GetActorRotation(), _targetRotation, _deltaTime, _rotationSpeed);
	...
	GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::White, 
		FString::Printf(TEXT("Rotation Speed: %.2f"), _rotationSpeed));
